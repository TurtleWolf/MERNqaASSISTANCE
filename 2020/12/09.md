...
1. [I need to know what is causing this bug This is the GitHub link. Kindly have a look at this project which I designed on my own from scratch after doing your task list project. This is the app link. After having a look it at, please try to find the reason behind the following bug: After adding few tasks to any day, when I delete some tasks, I do see in the local Storage that they are getting deleted. But there are few tasks in the task list which are not getting deleted from the local storage but are getting deleted from the DOM, thus on refreshing they are showing up again! Thanks Brad for your best ever explanations I find. It's after your teaching only that I am able to build such a kind of project on my own. I did courses earlier of other instructors but was never so confident to try a similar but different project than the what the course made me do. Thank you so very much.](https://www.udemy.com/course/modern-javascript-from-the-beginning/learn/lecture/8762218#questions/9729404 "Solid effort with lots of moving parts to keep track of.. right at the top I'd be curious why your array for weekdays has anything but weekdays in it. What ever you are trying to do with that first entry of weeks feels like it should be abstracted into it's own let and not mixed in with all of the others. let weekdays = ['weeks', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun']; also under storage, it seems odd that Tues would be an object when the others are arrays?")

1. [.forEach() Hello, Why does it print Hello:0 Hello:1 Hello:2 etc when we add the index parameter. Why doesn;t it do the same when we add 'li' in it. For eg: console.log(`Hello:${li}`); doesn't give the same output](https://www.udemy.com/course/modern-javascript-from-the-beginning/learn/lecture/8757270#questions/13405958 "starting at the 7:30 mark on line 20 he converts it to an array at 8:50 he's passing in the index on line 25 and then logging the Index. Hello comes from a string literal. On line 27 it's the index that was passed in followed by the string of `hello`. It's literally just doing what he has scripted for it to do, no more, no less.. he's using two console logs, one on line 26 in the forEach loop and then another after the loop at the end on line 30, but that one isn't told to log the index, or the hello string.. just the array of list items. I think your confusion comes from the forEach and not understanding the 3 default arguments that are built into it.Check out this link on MDN.. https://developer.mozilla.org/forEach specifically, the 3 arguments that are supplied by default. So when he passes li in on line 25, it is only defined locally for the next 2 lines and the only value it has is the index number. li above or below that line has no global value and those two lines have no other value assigned except for its local value of being the index passed to it from the array it is being called on.")

1. [Can you (or anyone else) please explain why our removeTaskFromLocalStorage() function does not remove the task when I use the following approach to create the li's in our addTask and getTasks functions:](https://www.udemy.com/course/modern-javascript-from-the-beginning/learn/lecture/8762208#questions/3601364 "If I'm reading this correctly, you'd be adding it Adjacent, which would be a sibling instead of a child. It's supposed to append a child, but instead, you'd be adding a brother, so you're logging the uncle.")
...
1. [Failed attempt to simplify code Can you (or anyone else) please explain why our removeTaskFromLocalStorage() function does not remove the task when I use the following approach to create the li's in our addTask and getTasks functions:](https://www.udemy.com/course/modern-javascript-from-the-beginning/learn/lecture/8762208#questions/3601364 "Google's Homepage")
...
## [React Front to Back](https://www.udemy.com/course/modern-javascript-from-the-beginning/ 'Node.js API Masterclass With Express & MongoDB')

### December 2020: React Front to Back
